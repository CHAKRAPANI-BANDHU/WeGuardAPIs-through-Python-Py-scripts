...
ListUrl = "enterprise/rest/weemm/product/list"
@pytest.mark.parametrize('url', [""])
@pytest.mark.skipif(config.run_test_tc_003_group_list== 0, reason="When group is not executed")
@pytest.mark.positivetest
@pytest.mark.policygroups
@pytest.mark.sanitytest
@pytest.mark.regressiontest
@pytest.mark.run(order=10081)
def test_tc_004_List(url):
    if globalvar.bearerToken == '':
        pytest.skip("Empty Bearer token Skipping test")
    try :
        apiUrl = globalvar.BaseURL + ListUrl
        print("\n\n--------------------------- List ---------------------------")
